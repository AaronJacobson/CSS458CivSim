...
class Game(object):
    ...
    def run(self):
        """
        """
        yield_vals = N.zeros((self.num_turns,len(self.civs),5))
        #Initialize run loop
                        ...
                        for entry in civ.wars:
                            ...
                            if self.loss_chance*(1/rel_strength) > N.random.uniform():
                                if len(civ.city_list) > 0:
                                    civ.city_list[-1].pop = max((civ.city_list[-1].pop//2,1))
                            ...
                            if self.loss_chance*(rel_strength) > N.random.uniform():
                                entry[0].city_list[-1].pop = max((entry[0].city_list[-1].pop//2,1))
